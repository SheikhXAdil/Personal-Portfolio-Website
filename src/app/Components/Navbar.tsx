...
export default function Navbar() {
    ...
    return (
        <nav className='sm:px-0 w-full flex flex-col md:flex-row justify-between'>
            ...
            <div id="navbar" className="hidden my-6 bg-[#292750] md:bg-transparent md:my-0 md:block">
                <ul id="navbar" className='md:mx-0 list-none w-full md:w-auto text-center md:text-start flex flex-col md:flex-row md:justify-between md:gap-6'>
                    {navs.map((nav) => {
                        return (
                            <li className=' py-4 md:my-0 p-3 text-lg rounded-2xl font-medium text-white hover:bg-[#15133c] hover:text-[#EC994B] cursor-pointer'>
                                <a href={`#${nav}`}>{nav}</a>
                            </li>
                        )
                    })}
